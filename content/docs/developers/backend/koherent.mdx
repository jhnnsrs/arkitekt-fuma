...
Koherent adds **provenance-aware audit logging** to Django services that expose a
...
*which client/app* they used, and the verified **task** (assignation) it happened under.

It exposes those answers directly in your GraphQL schema as a queryable, filterable `provenance`
field.

<GithubRepo url="https://github.com/jhnnsrs/koherent" title="Koherent" />

<Callout type="info" title="Concepts first?">
This page is the hands-on integration guide. For the *why* — what provenance is, how the layers
stack, and how a single task groups many changes — read
[Provenance & Audit Logging](/docs/design/architecture/provenance) first.
</Callout>
...
assignation, attested by a signed provenance token.
</Callout>
...
- **Context types** — `KoherentExtension` switches on kante's `HttpContext` and `WsContext`. For
  ...
  of the operation; for websockets it does not (the connection — and its headers — is shared
