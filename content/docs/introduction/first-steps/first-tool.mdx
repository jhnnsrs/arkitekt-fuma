...
description: Learn what Apps, Plugins, and Nodes are in Arkitekt by installing your first plugin app and using a node to convert a file.
---
...
- Understand Nodes and how to use them
- Convert a file
...
<img src="/docs/introduction/first_steps/node_explaination.png" alt="The concept of a Node in Arkitekt" />
...
anything special. Now that the app is deployed and running, every node it provides is **immediately available** —
just find the node you need and assign your data to it directly.

<Callout type="info" title="It used to be harder">


If you used an older version of Arkitekt, you might remember having to _reserve_ a node first — booking it before
you could use it. We got rid of that entirely. Now an action is simply usable whenever an app (an _agent_) that
implements it is connected. Isn't this so much easier now?


</Callout>

That's really all there is to it. In the next section we'll find the `Convert Omero` node, point our file at it,
