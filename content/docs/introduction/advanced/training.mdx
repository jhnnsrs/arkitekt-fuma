...
- You know the drill. Search for the `Train CARE` node and select it to make it available.
...
For this we will now use the `Predict CARE` node, which in this case will take a `Model` and an `Image` as input and return an image.
